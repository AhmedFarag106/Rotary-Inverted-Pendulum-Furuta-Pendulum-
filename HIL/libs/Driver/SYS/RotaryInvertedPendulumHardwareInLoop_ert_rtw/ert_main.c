...
void rt_OneStep(void)
{
  ...
  currentTime = (extmodeSimulationTime_T)
    RotaryInvertedPendulumHardwa_M->Timing.clockTick0;
  RotaryInvertedPendulumHardwareInLoop_step();
...
int main(void)
{
  ...
  init();
  MW_Arduino_Init();
  rtmSetErrorStatus(RotaryInvertedPendulumHardwa_M, 0);
  ...
  RotaryInvertedPendulumHardwareInLoop_initialize();
  cli();
  sei();

  /* External Mode initialization */
  errorCode = extmodeInit(RotaryInvertedPendulumHardwa_M->extModeInfo,
    &rtmGetTFinal(RotaryInvertedPendulumHardwa_M));
  if (errorCode != EXTMODE_SUCCESS) {
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(RotaryInvertedPendulumHardwa_M, true);
    }
  }
  ...
  runModel = !extmodeSimulationComplete() && !extmodeStopRequested() &&
    !rtmGetStopRequested(RotaryInvertedPendulumHardwa_M);
  ...
  /* Terminate model */
  RotaryInvertedPendulumHardwareInLoop_terminate();
